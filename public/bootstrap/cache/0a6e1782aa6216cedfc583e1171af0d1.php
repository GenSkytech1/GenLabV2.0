...
<div class="content">
    ...
    <div class="card">
      <div class="card-body">
  <form method="GET" action="<?php echo e(route('superadmin.cheques.index')); ?>" class="d-flex flex-nowrap align-items-end justify-content-between gap-2 mb-3 filter-toolbar">
          ...
          <div class="flex-grow-1" style="max-width: 340px; min-width: 180px;">
            <div class="input-group filter-search rounded-pill shadow-sm">
              ...
              <input type="text" class="form-control border-0 bg-transparent" name="q" value="<?php echo e($filters['q'] ?? ''); ?>" placeholder="Search...">
          ...
          <div class="d-flex flex-nowrap align-items-end gap-2" style="flex-shrink:0;">
            <select class="form-select" name="month" style="min-width: 160px;">
              ...
              <?php for($m=1;$m<=12;$m++): ?>
                <option value="<?php echo e($m); ?>" <?php if(($filters['month'] ?? '')==$m): echo 'selected'; endif; ?>><?php echo e(date('F', mktime(0,0,0,$m,1))); ?></option>
              <?php endfor; ?>
            </select>
  ...
  <div class="row g-2 mb-3">
          <div class="col-auto">
            <a href="<?php echo e(route('superadmin.cheques.index', ['status' => 'issued'])); ?>" class="btn btn-light border <?php echo e(($status ?? '')==='issued' ? 'active' : ''); ?>">Issued</a>
          </div>
          <div class="col-auto">
            <a href="<?php echo e(route('superadmin.cheques.index', ['status' => 'received'])); ?>" class="btn btn-light border <?php echo e(($status ?? '')==='received' ? 'active' : ''); ?>">Received</a>
          </div>
        </div>
        ...
        <div class="table-responsive">
          <table class="table table-hover align-middle">
            ...
            <tbody>
              <?php $__empty_1 = true; $__currentLoopData = $cheques; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $chq): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php if(($status ?? '') === 'received'): ?>
                  <tr>
                    ...
                    <td>
                      <form method="POST" action="<?php echo e(route('superadmin.cheques.toggleDeposit', $chq)); ?>">
                        <?php echo csrf_field(); ?>
                        <button class="btn btn-sm <?php echo e($chq->deposit_status ? 'btn-success' : 'btn-outline-secondary'); ?>" type="submit"><?php echo e($chq->deposit_status ? 'Yes' : 'No'); ?></button>
                      </form>
                    </td>
                    <td>
                      <?php if($chq->received_copy_path): ?>
                        <button type="button" class="btn btn-sm btn-outline-primary btn-view-copy" data-url="<?php echo e(asset('storage/'.$chq->received_copy_path)); ?>">View</button>
                      <?php else: ?>
                ...
                <?php else: ?>
                  <tr>
                    <td><?php echo e($cheques->firstItem() + $i); ?></td>
                    <td><?php echo e($chq->cheque_no); ?></td>
                    <td><?php echo e($chq->bank); ?></td>
                    <td><?php echo e($chq->payee_name); ?></td>
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="issueChequeForm" method="POST" action="<?php echo e(route('superadmin.cheques.store')); ?>" autocomplete="off">
          <?php echo csrf_field(); ?>
          <div class="row g-3">
...
<div class="modal fade" id="receiveChequeModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="receiveChequeForm" method="POST" action="<?php echo e(route('superadmin.cheques.storeReceived')); ?>" enctype="multipart/form-data">
          <?php echo csrf_field(); ?>
          <div class="row g-3">
            <div class="col-md-6">
...
(function() {
  ...
  // Bank suggestions
  const banks = <?php echo json_encode(($templateBanks ?? [])->pluck('bank_name'), 15, 512) ?>;
...
<?php echo $__env->make('superadmin.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH A:\GenTech\htdocs\GenlabV1.0\GenLabV1.0\resources\views/superadmin/accounts/cheques/index.blade.php ENDPATH**/ ?>