...
use App\Models\{NewBooking, Department, Invoice, InvoiceBookingItem, PaymentSetting, SiteSetting};
...
class GenerateInvoiceStatusController extends Controller
{
    ...
    public function index(Request $request, Department $department = null)
    {
        ...
        $view = ($request->payment_option ?? 'bill') === 'bill'
            ...
            : 'superadmin.cashPayments.index';

        return view($view, compact('bookings', 'department', 'departments'))
            ->with([
                'search' => $request->search,
                'month' => $request->month,
                'year' => $request->year,
    ...
    public function bulkGenerate(Request $request)
    {
        ...
        if (empty($bookingIds)) {
            return redirect()
                ...
                ->with('error', 'Please select at least one booking.');
        }
        ...
        if ($bookings->isEmpty()) {
            return redirect()
                ->route('superadmin.bookingInvoiceStatuses.index')
                ->with('error', 'No valid bookings found.');
        } 
        ...
        if ($uniqueClientIds->count() > 1) {
            return redirect()
                ->back()
                ->with('error', 'Selected bookings must belong to the same client.');
        }
        ...
        if ($uniqueMarketingIds->count() > 1) {
            return redirect()
                ->back()
                ->with('error', 'Selected bookings must have the same marketing person.');
        }
